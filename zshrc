...
setopt prompt_subst	# do param/arithm expansion and cmd subst in prompt
...
%{$terminfo[bold]$fg[green]%}%m%{$terminfo[sgr0]%} \
...
# enable color support of ls & friends (see .aliasrc)
if [ "$TERM" != "dumb" ]; then
    eval "`dircolors -b`"
